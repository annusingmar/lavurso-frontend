<template>
    <q-drawer show-if-above :model-value="open" side="left" bordered elevated>
        ...
        <q-list>
            <DrawerListItem v-for="(item, index) in menuItems" :title="item.title" :icon="item.icon" :to="item.to"
                :separator="item.separator" :key="index"></DrawerListItem>
            <template v-if="userRole === 'admin' || userRole === 'teacher'">
                <DrawerListItem v-for="(item, index) in teacherMenuItems" :title="item.title" :icon="item.icon"
                    :to="item.to" :separator="item.separator" :key="index"></DrawerListItem>
            </template>
            <template v-if="userRole === 'admin'">
                <DrawerListItem v-for="(item, index) in adminMenuItems" :title="item.title" :icon="item.icon"
                    :to="item.to" :separator="item.separator" :key="index"></DrawerListItem>
            </template>
        </q-list>
    </q-drawer>
